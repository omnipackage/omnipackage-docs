---
description: "`omnipackage build` reference — build RPM, DEB, and pacman packages for the distros in config.yml without publishing."
---

# `omnipackage build`

Build packages for the distros defined in `.omnipackage/config.yml` without publishing. A successful run produces `.rpm` / `.deb` / `.pkg.tar.zst` files under `--build-dir`, ready for [`publish`](publish.md).

```
omnipackage build [project-dir] [flags]
```

`project-dir` defaults to `.`, so plain `omnipackage build` works from the project root.
...
| `--container-output <stderr|stdout|null>` | `stderr` | Where output from the build process inside the container is **printed to the terminal**. `null` means nothing is printed. OmniPackage's own logs always go to stdout, so the default keeps the streams separate. The full container log is **always** written to a file under `--build-dir` regardless |
