...
- **Redaction only applies to OmniPackage's own log output.** Files written *into* the build (a rendered spec containing {% raw %}`{{ secrets.X }}`{% endraw %}) keep the literal value — that is what makes them work. Treat the build directory the same way you would treat the GPG key on disk.
