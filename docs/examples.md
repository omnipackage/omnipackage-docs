...
Minimal single-purpose projects live in the [examples repo]({{ examples_github_url }}), one per language/build-system combination: `c_makefile`, `crystal`, `electron`, `go`, `python`, `ruby`, `rust`, `tauri`, `c_with_secrets`. Each ships a ready-to-use `.omnipackage/config.yml` (RPM, DEB, and pacman targets) and a README.
